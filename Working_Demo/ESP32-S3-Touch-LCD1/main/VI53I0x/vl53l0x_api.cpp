#ifndef VL53L0X_FULL_INCLUDE_H
#define VL53L0X_FULL_INCLUDE_H
...
#include "vl53l0x_register_settings.h"
...
VL53L0X_Error VL53L0X_StaticInit(VL53L0X_DEV Dev)
{
	...
	pTuningSettingBuffer = VL53L0X_DefaultTuningSettings;
	...
	if ((ApertureSpads > 1) ||
		...
		Status = VL53L0X_SetReferenceSpads(Dev, count, ApertureSpads);
	...
	if (Status == VL53L0X_ERROR_NONE)
		Status = VL53L0X_LoadTuningSettings(Dev,
						      pTuningSettingBuffer);
	...
	/* Set interrupt config to new sample ready */
 	if (Status == VL53L0X_ERROR_NONE) {
		...
		VL53L0X_GPIO_FUNCTIONALITY_OFF,
	...
	if (Status == VL53L0X_ERROR_NONE) {
		for (i = 0; i < VL53L0X_DMAX_LUT_SIZE; i++) {
			pDeviceParameters->dmax_lut.ambRate_mcps[i] =
			   ((VL53L0X_Dev_t*)Dev)->Data.CurrentParameters.dmax_lut.ambRate_mcps[i];
			pDeviceParameters->dmax_lut.dmax_mm[i] =
			   ((VL53L0X_Dev_t*)Dev)->Data.CurrentParameters.dmax_lut.dmax_mm[i];
		}
	}
	...
	VL53L0X_SequenceStepId SequenceStepId, FixPoint1616_t TimeOutMilliSecs)
{
	 ...
	 */
	Status = VL53L0X_GetSequenceStepTimeout (Dev, SequenceStepId,
		&OldTimeOutMicroSeconds);

	if (Status == VL53L0X_ERROR_NONE) {
		Status = VL53L0X_SetSequenceStepTimeout(Dev, SequenceStepId,
			TimeoutMicroSeconds);
	}
	...
	if (Status == VL53L0X_ERROR_NONE) {
		...
		if (Status != VL53L0X_ERROR_NONE) {
			Status1 = VL53L0X_SetSequenceStepTimeout(Dev, SequenceStepId,
				OldTimeOutMicroSeconds);
	...
	FixPoint1616_t *pTimeOutMilliSecs)
{
	...
	Status = VL53L0X_get_sequence_step_timeout(Dev, SequenceStepId,
		&TimeoutMicroSeconds);
	if (Status == VL53L0X_ERROR_NONE) {
	...
	uint8_t StartNotStopFlag)
{
	...
	case VL53L0X_GPIO_FUNCTIONALITY_THRESHOLD_CROSSED_LOW:
		...
		break;
	case VL53L0X_GPIO_FUNCTIONALITY_THRESHOLD_CROSSED_HIGH:
		Status = VL53L0X_GetInterruptThresholds(Dev,
					VL53L0X_DEVICEMODE_CONTINUOUS_RANGING,
					&ThresholdLow, &ThresholdHigh);
		...
		break;
	case VL53L0X_GPIO_FUNCTIONALITY_THRESHOLD_CROSSED_OUT:
		Status = VL53L0X_GetInterruptThresholds(Dev,
					VL53L0X_DEVICEMODE_CONTINUOUS_RANGING,
					&ThresholdLow, &ThresholdHigh);
	...
	uint8_t *pMeasurementDataReady)
{
	...
	if (InterruptConfig ==
		VL53L0X_GPIO_FUNCTIONALITY_NEW_MEASURE_READY) {
...
Status |= VL53L0X_get_pal_range_status(Dev,
	...
	VL53L0X_InterruptPolarity Polarity)
{
	...
	if (Pin != 0) {
		...
		if (Status == VL53L0X_ERROR_NONE) {
			...
			case VL53L0X_GPIO_FUNCTIONALITY_OFF:
				data = 0x00;
				break;
			case VL53L0X_GPIO_FUNCTIONALITY_THRESHOLD_CROSSED_LOW:
				data = 0x01;
			...
			case VL53L0X_GPIO_FUNCTIONALITY_THRESHOLD_CROSSED_OUT:
				data = 0x03;
				break;
			case VL53L0X_GPIO_FUNCTIONALITY_NEW_MEASURE_READY:
				data = 0x04;
				break;
			default:
				Status =
...
{
	VL53L0X_Error Status = VL53L0X_ERROR_NONE;
	VL53L0X_GpioFunctionality GpioFunctionality = VL53L0X_GPIO_FUNCTIONALITY_OFF;
	uint8_t data;
	...
	if (Status == VL53L0X_ERROR_NONE) {
		switch (data & 0x07) {
		case 0x00:
			GpioFunctionality = VL53L0X_GPIO_FUNCTIONALITY_OFF;
			break;
		case 0x01:
			GpioFunctionality =
